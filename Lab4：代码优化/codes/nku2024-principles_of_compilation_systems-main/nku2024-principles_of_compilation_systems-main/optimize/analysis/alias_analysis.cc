...
bool SimpleAliasAnalyser::IsSamePtrWithDiffConstIndex(Operand p1, Operand p2, CFG *C) {
    if (p1->GetOperandType() == BasicOperand::REG && p2->GetOperandType() == BasicOperand::REG) {
        ...
        if (I1 != nullptr && I2 != nullptr && I1->GetOpcode() == BasicInstruction::GETELEMENTPTR && I2->GetOpcode() == BasicInstruction::GETELEMENTPTR) {
...
void SimpleAliasAnalyser::AliasAnalysis(CFG *C) {
    ...
    for (int i = 0; i < C->function_def->formals.size(); ++i) {
        ...
        if (param == BasicInstruction::LLVMType::PTR) {
        ...
        while (!q.empty()) {
            ...
            for (auto I : BB->Instruction_list) {
                ...
                if (I->GetOpcode() == BasicInstruction::GETELEMENTPTR) {
                    ...
                    if (PhiI->GetDataType() == BasicInstruction::PTR) {
...
void DCEPass::SimpleAliasAnalysis(LLVMIR *IR) {
    // std::cout<<"DCEPass::SimpleAliasAnalysis call"<<std::endl;
