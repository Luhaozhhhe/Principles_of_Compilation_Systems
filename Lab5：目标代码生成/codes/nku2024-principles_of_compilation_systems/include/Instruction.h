...
public:
    ...
    virtual void ReplaceReg(const std::map<int, int>& Mem2RegMAP)=0;
...
public:
    ...
    void ReplaceReg(const std::map<int, int>& Mem2RegMAP)
    {
        if (pointer->GetOperandType() == BasicOperand::REG) {
            auto pointer_reg = (RegOperand *)pointer;
            if (Mem2RegMAP.find(pointer_reg->GetRegNo()) != Mem2RegMAP.end()){
                this->pointer = GetNewRegOperand(Mem2RegMAP.find(pointer_reg->GetRegNo())->second);
            }
        }
        if (result->GetOperandType() == BasicOperand::REG) {
            auto result_reg = (RegOperand *)result;
            if (Mem2RegMAP.find(result_reg->GetRegNo()) != Mem2RegMAP.end()){
                this->result = GetNewRegOperand(Mem2RegMAP.find(result_reg->GetRegNo())->second);
            }
        }
    }
...
public:
    ...
    void ReplaceReg(const std::map<int, int>& Mem2RegMAP)
    {
        ...
        if (value->GetOperandType() == BasicOperand::REG) {
            ...
            if (Mem2RegMAP.find(value_reg->GetRegNo()) != Mem2RegMAP.end()){
                this->value = GetNewRegOperand(Mem2RegMAP.find(value_reg->GetRegNo())->second);
            }
        }
    }
...
public:
    ...
    void ReplaceReg(const std::map<int, int>& Mem2RegMAP){
        ...
        if (op2->GetOperandType() == BasicOperand::REG) {
            ...
            if (Mem2RegMAP.find(op2_reg->GetRegNo()) != Mem2RegMAP.end()){
                this->op2 = GetNewRegOperand(Mem2RegMAP.find(op2_reg->GetRegNo())->second);
            }
        }
        if (op1->GetOperandType() == BasicOperand::REG) {
            auto op1_reg = (RegOperand *)op1;
            if (Mem2RegMAP.find(op1_reg->GetRegNo()) != Mem2RegMAP.end()){
                this->op1 = GetNewRegOperand(Mem2RegMAP.find(op1_reg->GetRegNo())->second);
            }
        }
        if (result->GetOperandType() == BasicOperand::REG) {
            auto result_reg = (RegOperand *)result;
    ...
    void ReplaceReg(const std::map<int, int>& Mem2RegMAP){
        for (auto &label_pair : phi_list) {
            ...
            if (op1->GetOperandType() == BasicOperand::REG) {
                auto op1_reg = (RegOperand *)op1;
                if (Mem2RegMAP.find(op1_reg->GetRegNo()) != Mem2RegMAP.end())
                    op1 = GetNewRegOperand(Mem2RegMAP.find(op1_reg->GetRegNo())->second);
            }
            auto &op2 = label_pair.second;
            if (op2->GetOperandType() == BasicOperand::REG) {
                auto op2_reg = (RegOperand *)op2;
                if (Mem2RegMAP.find(op2_reg->GetRegNo()) != Mem2RegMAP.end())
                    op2 = GetNewRegOperand(Mem2RegMAP.find(op2_reg->GetRegNo())->second);
            }
        }
        if (result->GetOperandType() == BasicOperand::REG) {
            auto result_reg = (RegOperand *)result;
...
public:
    ...
    void ReplaceReg(const std::map<int, int>& Mem2RegMAP){
        if (cond->GetOperandType() == BasicOperand::REG) {
            ...
            if (Mem2RegMAP.find(cond_reg->GetRegNo()) != Mem2RegMAP.end())
                this->cond = GetNewRegOperand(Mem2RegMAP.find(cond_reg->GetRegNo())->second);
        }
    }
    void ReplaceLable(const std::map<int, int>& Mem2RegMAP){
        auto true_label = (LabelOperand *)this->trueLabel;
        auto false_label = (LabelOperand *)this->falseLabel;

        if (Mem2RegMAP.find(true_label->GetLabelNo()) != Mem2RegMAP.end()) {
            trueLabel = GetNewLabelOperand(Mem2RegMAP.find(true_label->GetLabelNo())->second);
        }

        if (Mem2RegMAP.find(false_label->GetLabelNo()) != Mem2RegMAP.end()) {
            falseLabel = GetNewLabelOperand(Mem2RegMAP.find(false_label->GetLabelNo())->second);
        }
    }
    int GetResultRegNo() { return -1; }
    ...
    void ReplaceReg(const std::map<int, int>& Mem2RegMAP){
        for (auto &arg_pair : args) {
            if (arg_pair.second->GetOperandType() == BasicOperand::REG) {
                ...
                if (Mem2RegMAP.find(op->GetRegNo()) != Mem2RegMAP.end())
                    arg_pair.second = GetNewRegOperand(Mem2RegMAP.find(op->GetRegNo())->second);
            }
        }
        if (result != NULL) {
            if (result->GetOperandType() == BasicOperand::REG) {
    ...
    void ReplaceReg(const std::map<int, int>& Mem2RegMAP)
    {
        if (ret_val != NULL) {
            if (ret_val->GetOperandType() == BasicOperand::REG) {
                ...
                if (Mem2RegMAP.find(result_reg->GetRegNo()) != Mem2RegMAP.end()){
                    ret_val = GetNewRegOperand(Mem2RegMAP.find(result_reg->GetRegNo())->second);
                }
            }
        }
    }
...
public:
    ...
    void ReplaceReg(const std::map<int, int>& Mem2RegMAP){
        ...
        if (ptrval->GetOperandType() == BasicOperand::REG) {
            ...
            if (Mem2RegMAP.find(result_reg->GetRegNo()) != Mem2RegMAP.end())
                this->ptrval = GetNewRegOperand(Mem2RegMAP.find(result_reg->GetRegNo())->second);
        }
        for (auto &idx_pair : indexes) {
            if (idx_pair->GetOperandType() == BasicOperand::REG) {
                auto idx_reg = (RegOperand *)idx_pair;
                if (Mem2RegMAP.find(idx_reg->GetRegNo()) != Mem2RegMAP.end())
                    idx_pair = GetNewRegOperand(Mem2RegMAP.find(idx_reg->GetRegNo())->second);
            }
        }
    }
...
public:
    ...
    void ReplaceReg(const std::map<int, int>& Mem2RegMAP){
        ...
        if (value->GetOperandType() == BasicOperand::REG) {
            ...
            if (Mem2RegMAP.find(result_reg->GetRegNo()) != Mem2RegMAP.end())
                value = GetNewRegOperand(Mem2RegMAP.find(result_reg->GetRegNo())->second);
        }
    }
    Operand GetResultReg() { return result; }
    int GetResultRegNo() { return ((RegOperand *)result)->GetRegNo();}
...
public:
    Operand GetResult() { return result; }
    Operand GetSrc() { return value; }
    Operand GetDst() { return result; }//added
