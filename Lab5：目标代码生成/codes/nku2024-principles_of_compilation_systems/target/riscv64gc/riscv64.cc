...
std::vector<Register *> RiscV64Instruction::GetReadReg() {//不同类型的指令的获取
...
std::vector<Register *> RiscV64Instruction::GetWriteReg() {//不同类型的指令的编写
    switch (OpTable[op].ins_formattype) {
    case RvOpInfo::R_type:
...
Register RiscV64Spiller::GenerateReadCode(std::list<MachineBaseInstruction *>::iterator &it, int raw_stk_offset,
    ...
    // 这里没有考虑imm超限的情况，是因为后面经过lowerimm的过程，其实需要判断。
    // 其实就是计算一个偏移量作为地址存在reg里，然后直接load就行了。
    auto offset_mid_reg = function->GetNewRegister(INT64.data_type, INT64.data_length);
    cur_block->insert(it, rvconstructor->ConstructIImm(RISCV_ADDI, offset_mid_reg, GetPhysicalReg(RISCV_sp), offset));
    if (type == INT64) 
    {
