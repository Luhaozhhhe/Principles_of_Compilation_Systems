...
struct MachineDataType {//定义了数据类型和数据长度的枚举值
    enum { INT, FLOAT };
    enum { B32, B64, B128 };
    unsigned data_type;//数据类型的成员变量
    unsigned data_length;//数据长度的成员变量
    MachineDataType() {}
...
public:
    ...
    bool save_across_call;   //是否跨调用保存 added
    ...
    bool operator<(Register other) const { //比较两个 Register 对象的大小
...
struct MachineBaseOperand {
    ...
    //寄存器（REG）、整数立即数（IMMI）、单精度浮点数立即数（IMMF）和双精度浮点数立即数（IMMD）
    int op_type;
...
struct MachineRegister : public MachineBaseOperand {
    ...
    std::string toString() {
        if (reg.is_virtual)
            return "%" + std::to_string(reg.reg_no);//虚拟寄存器，前面用%
        else
            return "phy_" + std::to_string(reg.reg_no);//物理寄存器，前面用phy_
    }
};

struct MachineImmediateInt : public MachineBaseOperand {//将整数转换为字符串表示
    long long int imm32;
    MachineImmediateInt(long long int imm32) : MachineBaseOperand(MachineBaseOperand::IMMI), imm32(imm32) {}
    std::string toString() { return std::to_string(imm32); }
};
struct MachineImmediateFloat : public MachineBaseOperand {//将浮点数转换为字符串表示
    float fimm32;
...
struct MachineImmediateDouble : public MachineBaseOperand {//将双精度浮点数转换为字符串表示
    double dimm64;
...
class MachineBaseInstruction {//架构的列举
public:
    enum { ARM = 0, RiscV, PHI, NOP};
    const int arch;
