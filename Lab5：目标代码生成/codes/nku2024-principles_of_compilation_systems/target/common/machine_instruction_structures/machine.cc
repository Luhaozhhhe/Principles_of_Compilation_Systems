...
void MachineCFG::MakeEdge(int edg_begin, int edg_end) {//创建一条边
...
// Just modify CFG edge, no change on branch instructions
void MachineCFG::RemoveEdge(int edg_begin, int edg_end) {//移出一条边
    assert(block_map.find(edg_begin) != block_map.end());
    assert(block_map.find(edg_end) != block_map.end());
...
Register MachineFunction::GetNewRegister(int regtype, int reglength, bool save_across_call) {//获取一个新的寄存器，是虚拟的
...
MachineBlock *MachineFunction::InitNewBlock() {//初始化一个新的块，分配一个新的id
