...
int main(int argc, char **argv) {
    ...
    //std::cout << "simplifyCFGPass End" << std::endl;
    //llvmIR.printIR(fout);


    llvmIR.BuildCFG();
    // std::cout << "BuildCFG End" << std::endl;
    //llvmIR.printIR(fout);
    //std::cout << "printIR End" << std::endl;
    // 消除不可达基本块和指令在不开启O1的情况也需要进行，原因是这属于基本优化
    //DomAnalysis dom(&llvmIR);
    //dom.Execute();
    // std::cout<<"dom.exe end"<<std::endl;
    ...
    if (optimize_flag) {
        //DomAnalysis dom(&llvmIR);
        // std::cout<<"begin dce pass"<<std::endl;
        //DCEPass dce_pass(&llvmIR);
        // std::cout<<"in dce pass"<<std::endl;
        //dce_pass.Execute();

        // std::cout<<"end dce pass"<<std::endl;

        // std::cout<<"begin adce pass"<<std::endl;
        //ADCEPass adce_pass(&llvmIR);
        // std::cout<<"in adce pass"<<std::endl;
        //adce_pass.Execute();
        // std::cout<<"finish adce pass"<<std::endl;
